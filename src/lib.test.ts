...
import { truncate, formatSize, omit, redact, redactWithHint, pick, shortenPath, lastSegments, getSessionCwd, formatTzOffset, progName, parseDuration, classifyUserTurnKind, isSlashCommandContent, extractUserTurnText, classifyUserTurn, isUserTurn, getForkedFromSessionId, findForkSplit } from "./lib.ts";
...
describe("classifyUserTurnKind", () => {
  describe("hidden_tag", () => {
    test("system-reminder タグのみ → hidden_tag", () => {
      expect(classifyUserTurnKind("<system-reminder>do not do X</system-reminder>")).toBe("hidden_tag");
    });

    test("user-prompt-submit-hook タグのみ → hidden_tag", () => {
      expect(classifyUserTurnKind("<user-prompt-submit-hook>injected</user-prompt-submit-hook>")).toBe("hidden_tag");
    });

    test("local-command-stdout タグのみ → hidden_tag", () => {
      expect(classifyUserTurnKind("<local-command-stdout>output here</local-command-stdout>")).toBe("hidden_tag");
    });

    test("タグ + 短い ASCII 相槌 (残文字≤20) → hidden_tag", () => {
      expect(classifyUserTurnKind("<system-reminder>x</system-reminder> ok thanks")).toBe("hidden_tag");
    });

    test("複数タグ混在 → hidden_tag", () => {
      expect(
        classifyUserTurnKind("<system-reminder>a</system-reminder><local-command-stdout>b</local-command-stdout>"),
      ).toBe("hidden_tag");
    });
    ...
    test("残文字19文字（複数word ASCII）→ hidden_tag（≤20）", () => {
      ...
      expect(classifyUserTurnKind(`<system-reminder>x</system-reminder> aa bb cc dd ee ff g`)).toBe("hidden_tag");
    });

    test("残文字20文字（複数word ASCII）→ hidden_tag（境界内）", () => {
      expect(("aa bb cc dd ee ff gg").length).toBe(20);
      expect(classifyUserTurnKind(`<system-reminder>x</system-reminder> aa bb cc dd ee ff gg`)).toBe("hidden_tag");
    });

    test("残文字21文字（複数word ASCII）→ effective（閾値超過）", () => {
      expect(("aa bb cc dd ee ff ggg").length).toBe(21);
      expect(classifyUserTurnKind(`<system-reminder>x</system-reminder> aa bb cc dd ee ff ggg`)).toBe("effective");
    });
  });
  ...
  describe("hidden_tag にならない", () => {
    ...
    test("タグ + 20文字超の ASCII 実指示 (3 word 以上) → effective", () => {
      expect(
        classifyUserTurnKind("<system-reminder>x</system-reminder> please refactor the whole module now"),
      ).toBe("effective");
    });
    ...
    test("タグ + 20文字超の ASCII (元テキストは 3 word 以上) → effective", () => {
      ...
      expect(
        classifyUserTurnKind("<system-reminder>x</system-reminder> supercalifragilistic expialidocious"),
      ).toBe("effective");
    });

    test("タグ + 日本語残文字 → effective", () => {
      expect(classifyUserTurnKind("<system-reminder>x</system-reminder> これを直して")).toBe("effective");
    });
  });
...
describe("isSlashCommandContent (extract.ts と統一した slash 判定)", () => {
  ...
  test("XML 風スラッシュコマンド全体は slash", () => {
    expect(isSlashCommandContent("<command-name>/clear</command-name><command-args></command-args>")).toBe(true);
  });
...
describe("extractUserTurnText (isUserTurn と classifyUserTurn で同一本文を見る)", () => {
  ...
  test("複数 text ブロックは改行連結（情報欠落しない方向）", () => {
    expect(
      extractUserTurnText({ message: { content: [{ type: "text", text: "これは" }, { type: "text", text: "日本語の指示" }] } }),
    ).toBe("これは\n日本語の指示");
  });

  test("text 以外のブロックは無視", () => {
    expect(
      extractUserTurnText({ message: { content: [{ type: "tool_result", content: "x" }, { type: "text", text: "本文" }] } }),
    ).toBe("本文");
  });
...
describe("classifyUserTurn: slash 判定の誤検知防止 (修正1)", () => {
  ...
  test("真の slash コマンド entry は slash_only", () => {
    const entry = { type: "user", message: { content: "<command-name>/clear</command-name><command-args></command-args>" } };
...
describe("isUserTurn と classifyUserTurn の本文一致 (修正3)", () => {
  ...
  test("複数 text ブロックの先頭が通常テキストなら両者とも turn として扱う", () => {
    const entry = { type: "user", message: { content: [{ type: "text", text: "これは" }, { type: "text", text: "日本語の長い指示です" }] } };
...
describe("findForkSplit (fork 境界判定: findings の単一の正)", () => {
  ...
  test("単純な fork: 最初の非コピー user が境界", () => {
    const entries = [
      { type: "user", uuid: "c1", forkedFrom: { sessionId: "parent-abc" }, message: { content: "親" } },
      { type: "assistant", uuid: "c2", forkedFrom: { sessionId: "parent-abc" }, message: { content: [{ type: "text", text: "応答" }] } },
  ...
  test("境界直後に非 user（custom-title/assistant/system/file-history-snapshot）が挟まっても最初の user を採用", () => {
    const entries = [
      { type: "user", uuid: "c1", forkedFrom: { sessionId: "parent-xyz" }, message: { content: "親" } },
      // 非コピー（forkedFrom 無し）だが非 user の境界群
      { type: "system", subtype: "custom_title", uuid: "ct-1", content: "btw: x" },
      { type: "assistant", uuid: "auto-1", message: { content: [{ type: "text", text: "No response requested." }] } },
      { type: "system", subtype: "local_command", uuid: "sys-1", content: "<command-name>/fork</command-name>" },
  ...
  test("forkedFrom が複数の異なる親を指しても最初の値を採用", () => {
    const entries = [
      { type: "user", uuid: "c1", forkedFrom: { sessionId: "parent-A" }, message: { content: "x" } },
